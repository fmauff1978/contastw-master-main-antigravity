...
import { Mensal2025Component } from '../mensal2025/mensal2025.component';
import { Agreg3Service } from '../../../services/agreg3.service';

@Component({
  selector: 'app-mensal2026',
  imports: [CommonModule, SpinnerComponent],
  templateUrl: './mensal2026.component.html',
  styleUrl: './mensal2026.component.css',
  providers: [{ provide: LOCALE_ID, useValue: 'pt' }],
})
export class Mensal2026Component implements OnInit {
  ...
  async ngOnInit() {
    ...
    try {
      ...
      this.qtdTotalMaior = this.fonte0.filter(item =>
      (item?.ano2026?.total ?? 0) > ((item?.gd2025 ?? 0) * this.dd)
    ).length;

    this.tamanho = this.fonte0.length;

   
    console.log(this.qtdTotalMaior)

      this.calcularTotaisMensais();
      this.calcularTotaisPorModalidade();
       this.ag3.calcularProjecoes("BB9Wo3WLdeFanctk8fYH",
     'w9jfd3xBO3o12KycB8DZ');
      this.ag3.calcularProjecoes("7yLtE9Loqo2Q416w9ndK",
     '5J7bYFhDejZHDpYdcIv7');
  ...
  calcularTotaisPorModalidade() {
    ...
    meses.forEach(mes => {
      this.totaisOff[mes] = this.fonte0.reduce((sum, item) => {
        return sum + (item.mod_despesa === 'off' ? (item.ano2026?.[mes] || 0) : 0);
      }, 0);
      this.totaisCompromissada[mes] = this.fonte0.reduce((sum, item) => {
        return sum + (item.mod_despesa === 'compromissada' ? (item.ano2026?.[mes] || 0) : 0);
      }, 0);
      this.totaisGerenciavel[mes] = this.fonte0.reduce((sum, item) => {
        return sum + (item.mod_despesa === 'gerenciável' ? (item.ano2026?.[mes] || 0) : 0);
      }, 0);
